...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if ($_GET['reply'] == "false") {
        echo '
        <p class="my-0 ps-1">' . $time . '</p>
        <p><a class="text-decoration-none text-secondary ps-1"  href="thread.php?threadid=' . $tid . '&replyid=' . $cid . '&reply=true">Reply <i style="font-size:12px" class="fa">&#xf112;</i></a></p><hr class="text-secondary my-0 text-opacity-25">';
    }
    echo '  
    </div>
                  </div>';

    //* Form for reply to the comment
    if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] && $_GET['reply'] == "true" && $_GET['replyid'] == $cid) {
        echo '
        <div class="container my-2">
            <form class="pb-3 ps-5 my-1" action="partials/_replyhandle.php?threadid=' . $tid . '&cid=' . $cid . '" method="post">

                <div class="mt-2 mb-2">
                    <label for="reply" class="form-label">Type your reply</label>
                    <textarea class="form-control p-2" style="height:100px;" placeholder="Comment your reply" name="reply"
                        id="reply"></textarea>
                </div>
                <button type="submit" class="btn btn-dark mt-1">Reply</button>
            </form>
        </div>';
    }

    $sql2 = "SELECT * FROM `replies` WHERE `reply_on` = '$cid'";
    $result2 = mysqli_query($conn, $sql2);

    while ($row2 = mysqli_fetch_assoc($result2)) {
        $re_content = $row2['reply_content'];
        $re_time = $row2['reply_time'];
        $re_by = $row2['reply_by'];
        ...
        <div class="d-flex">
            ...
            <div class="flex-grow-1 ps-3 py-0 ms-5">
                <p class="text-secondary my-0">' . $re_by . '</p>
                <p class="my-0">' . $re_content . '</p>
                <p class="mb-2">' . $re_time . '</p>
                </div>
                </div>';
    }
    echo '<hr class="container text-secondary my-1 text-opacity-25">';
